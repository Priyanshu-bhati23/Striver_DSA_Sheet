...
int findtarget(vector<int>nums,int n,int target ){
...
while (low<=high){
    ...
    if(nums[mid]<=nums[high]){
        if(nums[mid]<=target && nums[high]>=target) return low=mid+1;
        else high=mid-1;
    }
...
int main(){
    ...
    int target=0;
    int n=nums.size();
    cout<<"index is"<<findtarget(nums,n,target)<<endl;
