...
//maxlensubarray with at mist 2 types of numbers
...
int fruitintobaslket(int n,vector<int> fruits){
    ...
    for(int i=0;i<n;i++){
        mp[fruits[i]]++;
        while(mp.size()>2){
            mp[fruits[start]]--;//decreasing frequency
            if(mp[fruits[start]]==0) mp.erase(fruits[start]);//removing from map
            start++;
        }
        if(mp.size()<=2) ans=max(ans,i-start+1);
    }
    return ans;
}
...
int n=fruits.size();
cout<<fruitintobaslket(n,fruits);
